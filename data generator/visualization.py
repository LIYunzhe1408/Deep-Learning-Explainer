...
from data_operation import transform_convert_p_v, transform_convert_s_v, transform_convert_1
...
def patches_vis(data,mask,i,path):
    ...
    plt.savefig(os.path.join(folder_path,str(i)+'.jpg'),dpi = 1000)
    plt.clf()
    plt.close()
...
def superpixel_vis(data,mask,order,path):
    ...
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    plt.savefig(os.path.join(folder_path,str(order)+'.jpg'),dpi = 1000)
...
def result_heatmap(masks, con_t, data,folder_path,filename):
    ...
    # shap.image_plot(a, X)
    shap.image_plot(a,X,show=False)
    plt.savefig(os.path.join(folder_path,filename+'.jpg'))
    plt.clf()
    plt.close()
