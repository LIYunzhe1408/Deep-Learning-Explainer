import argparse
import os
from data_operation import IMGDataset, transform_convert_save, load_csv_pair_tensor, PSDataset, read_csv_normal, \
    load_csv_patches_tensor, load_mask_component, load_mask_object, read_csv_int, read_csv_np, get_start_and_end
import torchvision.models as models
from torch.utils.data.dataloader import DataLoader
import torch
from visualization import patches_vis, superpixel_vis, result_heatmap
import numpy as np
import skimage.segmentation as segmentation
from PathManager import Path


def two_stage_image_segmentation(pathset):
    """
    图片预处理模块，预处理图片保存在
    ...
    for i, data in enumerate(data_loader):
        ...
        # 语义分割结果可视化+保存
        patches_vis(data, output.detach(), i, pathset.dir_patch_image)
        ...
        # seg_masks_batch=np.array(seg_masks_batch)
        # print(num_non_zero)
        # max_mask_num=np.max(num_non_zero)
        # print(max_mask_num)
...
seg_save_param = 0
seg_batch_size = 10
source_image_set_id = "n02123045"
path = Path("tabby", "deeplabv3", source_image_set_id)
two_stage_image_segmentation(pathset=path)
