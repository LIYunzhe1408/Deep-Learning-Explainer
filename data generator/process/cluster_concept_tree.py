...
def hierarchical_class_concept_tree(pathManager, config):
    ...
    cluster_images = {}
    for i in range(n):
        patch_cluster = [data_patch_fn[j] for j in [np.where(ob_label == i)[0]]][0]
        print(len(patch_cluster), patch_cluster)
        cluster_images[i] = {"patch": list(patch_cluster), "superpixel": []}
    ...
    for i in range(num_p1):
        ...
        for k in range(class_count, class_count+n):
            ...
            cluster_images[i]["superpixel"].append(list(patch_cluster1))
    ...
    for key in cluster_images.keys():
        print(key, ": ", len(cluster_images[key]["superpixel"]))

    print(cluster_images)
    # 保存概念从属关系
    if isinstance(cluster_images, str):
        cluster_images = eval(cluster_images)
    with open(pathManager.dir_cluster_image, 'w', encoding='utf-8') as f:
        f.write(str(cluster_images))  # dict to str
    with open(pathManager.dir_cluster_image, 'r', encoding='utf-8') as f:
        dict_cluster = eval(f.read())  # eval
        print(dict_cluster == cluster_images)
